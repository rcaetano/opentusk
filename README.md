...
After building, enable sandboxing in `~/.openclaw/openclaw.json`:
...
Config in `~/.openclaw` is a bind mount on the host — it is **never** inside the Docker image and is preserved across rebuilds and restarts.
...
- **`~/.openclaw/config.env`** — Written by `mustangclaw init`. Controls ports, DigitalOcean settings.
- **`~/.openclaw/openclaw.json`** — Written by `mustangclaw setup`. Controls API keys, models, gateway auth (may use `auth.mode: "password"` or `auth.mode: "token"`). Mounted into the Docker container at `/home/node/.openclaw/`.
- **`~/.openclaw/devices/`** — Device pairing state (paired.json, pending.json). Goes stale on gateway restarts. Auto-cleared by `mustangclaw run`, `tui`, and `dashboard`.
...
> **Path mapping**: Host `~/.openclaw` is mounted into the Docker container as `/home/node/.openclaw`.
...
| Dashboard "device token mismatch" | Clear `~/.openclaw/devices/`, restart gateway, and clear browser localStorage for `localhost:18789` |
...
| TUI "gateway not connected" | Ensure gateway is running (`mustangclaw status`), then clear `~/.openclaw/devices/` |
