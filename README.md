...
> **Path mapping**: Host `~/.mustangclaw` is mounted into the Docker container as `/home/node/.openclaw`. The `.mustangclaw` name is host-only; inside the container everything lives under `.openclaw`.
