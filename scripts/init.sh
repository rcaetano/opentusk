...
# ─── Section: Webhook ──────────────────────────────────────────────────
printf "${_GREEN}── Auto-update Webhook ──${_NC}\n"
_WH_ENABLED="false"
_WH_SECRET=""
ask_yn _WH_ENABLED "Enable auto-update webhook (rebuilds Poseidon on push to main)?" "n"

if [[ "$_WH_ENABLED" == "true" ]]; then
    _WH_SECRET=$(openssl rand -hex 32)
    echo "  Generated webhook secret: ${_WH_SECRET:0:16}..."
fi
echo ""
...
if [[ "$_WH_ENABLED" == "true" ]]; then
    echo ""
    echo "  Webhook: enabled (auto-update on push to main)"
    echo ""
    echo "  After deploy, configure in your Poseidon repo (GitHub > Settings > Webhooks):"
    echo "    URL:          http://<droplet-ip>:18792/webhook"
    echo "    Content type: application/json"
    echo "    Secret:       $_WH_SECRET"
