...
- **Gateway crash-loop ("port 18789 already in use")**: A stale gateway process (often running as root) is holding the port. Fix: `kill -9 <pid>` (find it with `lsof -i :18789`), then `systemctl restart openclaw`. The systemd service runs as `openclaw` user, but a root-owned process can linger from manual runs or upgrades
- **Poseidon shows offline / "EACCES: permission denied" on sessions.json**: Files under `/home/openclaw/.openclaw/` ended up owned by root. Fix: `chown -R openclaw:openclaw /home/openclaw/.openclaw/` then `systemctl restart poseidon`. This can happen when the gateway or other processes run as root and write to the openclaw user's home directory
