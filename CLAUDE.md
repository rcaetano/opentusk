...
./mustangclaw setup     # run OpenClaw's interactive wizard (API keys, models, auth)
...
scripts/
  ...
  build.sh               # git clone + docker build
...
Written by `mustangclaw setup` (OpenClaw's own wizard). Controls API keys, models, gateway auth. Mounted into the Docker container at `/home/node/.openclaw/`.
...
- The `.env` file at `openclaw/.env` bridges our `MUSTANGCLAW_*` variables to the `OPENCLAW_*` names that docker-compose expects
